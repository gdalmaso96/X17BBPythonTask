...
def argparser():
    ...
    parser.add_argument('-BKGonly', '--BKGonly', type=bool, default=False, help='Plot only background')
...
if __name__ == '__main__':
    ...
    if not argparser().BKGonly:
            ...
            with open(result, 'r') as f:
                for line in f:
                    if line[0] == '#':
                        ...
                        nSig.append(float(values[0]))
                        nSigErr.append(float(values[1]))
                        nI176.append(float(values[2]))
                        nI176Err.append(float(values[3]))
                        nI179.append(float(values[4]))
                        nI179Err.append(float(values[5]))
                        nI181.append(float(values[6]))
                        nI181Err.append(float(values[7]))
                        nI146.append(float(values[8]))
                        nI146Err.append(float(values[9]))
                        nI149.append(float(values[10]))
                        nI149Err.append(float(values[11]))
                        nI151.append(float(values[12]))
                        nI151Err.append(float(values[13]))
                        nEpc18.append(float(values[14]))
                        nEpc18Err.append(float(values[15]))
                        nEpc15.append(float(values[16]))
                        nEpc15Err.append(float(values[17]))
                        fval.append(float(values[18]))
                        valid.append(bool(values[19]))
                        fvalH0.append(float(values[20]))
                        validH0.append(bool(values[21]))
                        lratio.append(float(values[22]))
                        pvalue.append(float(values[23]))
                        sigma.append(float(values[24]))
                        ExecTime.append(float(values[25]))
                        ExecTimeH0.append(float(values[26]))
                        if len(values) > 27:
                            mX17.append(float(values[27]))
        
        # Convert to numpy arrays
        nSig = np.array(nSig)
        nSigErr = np.array(nSigErr)
        nI176 = np.array(nI176)
        nI176Err = np.array(nI176Err)
        nI179 = np.array(nI179)
        nI179Err = np.array(nI179Err)
        nI181 = np.array(nI181)
        nI181Err = np.array(nI181Err)
        nI146 = np.array(nI146)
        nI146Err = np.array(nI146Err)
        nI149 = np.array(nI149)
        nI149Err = np.array(nI149Err)
        nI151 = np.array(nI151)
        nI151Err = np.array(nI151Err)
        nEpc18 = np.array(nEpc18)
        nEpc18Err = np.array(nEpc18Err)
        nEpc15 = np.array(nEpc15)
        nEpc15Err = np.array(nEpc15Err)
        ...
        plt.suptitle(f'{prefix}, {len(nSig)} tests')
        plt.subplot(2, 4, 1)
        H = plt.hist(nI176/np.mean(nI176), bins=50, label=f'mean = {np.mean(nI176):.2f}, std = {np.std(nI176):.2f},\n relative std = {1e2*np.std(nI176)/np.mean(nI176):.2f} %', color=cm.coolwarm(0.))
        #print(H)
        plt.stairs(H[0], H[1], color=cm.coolwarm(0.), linewidth=8)
        plt.xlabel(r'$\mathcal{N}_{\mathrm{IPC17.6}}/\hat{\mathcal{N}}_{\mathrm{IPC17.6}}$')
        plt.legend()
        plt.grid()
        
        plt.subplot(2, 4, 2)
        H = plt.hist(nI179/np.mean(nI179), bins=50, label=f'mean = {np.mean(nI179):.2f}, std = {np.std(nI179):.2f},\n relative std = {1e2*np.std(nI179)/np.mean(nI179):.2f} %', color=cm.coolwarm(0.))
        plt.stairs(H[0], H[1], color=cm.coolwarm(0.), linewidth=8)
        plt.xlabel(r'$\mathcal{N}_{\mathrm{IPC17.9}}/\hat{\mathcal{N}}_{\mathrm{IPC17.9}}$')
        plt.legend()
        plt.grid()
        
        plt.subplot(2, 4, 3)
        H = plt.hist(nI181/np.mean(nI181), bins=50, label=f'mean = {np.mean(nI181):.2f}, std = {np.std(nI181):.2f},\n relative std = {1e2*np.std(nI181)/np.mean(nI181):.2f} %', color=cm.coolwarm(0.))
        plt.stairs(H[0], H[1], color=cm.coolwarm(0.), linewidth=8)
        plt.xlabel(r'$\mathcal{N}_{\mathrm{IPC18.1}}/\hat{\mathcal{N}}_{\mathrm{IPC18.1}}$')
        plt.legend()
        plt.grid()
        
        plt.subplot(2, 4, 4)
        H = plt.hist(nI146/np.mean(nI146), bins=50, label=f'mean = {np.mean(nI146):.2f}, std = {np.std(nI146):.2f},\n relative std = {1e2*np.std(nI146)/np.mean(nI146):.2f} %', color=cm.coolwarm(0.))
        plt.stairs(H[0], H[1], color=cm.coolwarm(0.), linewidth=8)
        plt.xlabel(r'$\mathcal{N}_{\mathrm{IPC14.6}}/\hat{\mathcal{N}}_{\mathrm{IPC14.6}}$')
        plt.legend()
        plt.grid()
        
        plt.subplot(2, 4, 5)
        H = plt.hist(nI149/np.mean(nI149), bins=50, label=f'mean = {np.mean(nI149):.2f}, std = {np.std(nI149):.2f},\n relative std = {1e2*np.std(nI149)/np.mean(nI149):.2f} %', color=cm.coolwarm(0.))
        plt.stairs(H[0], H[1], color=cm.coolwarm(0.), linewidth=8)
        plt.xlabel(r'$\mathcal{N}_{\mathrm{IPC14.9}}/\hat{\mathcal{N}}_{\mathrm{IPC14.9}}$')
        plt.legend()
        plt.grid()
        
        plt.subplot(2, 4, 6)
        H = plt.hist(nI151/np.mean(nI151), bins=50, label=f'mean = {np.mean(nI151):.2f}, std = {np.std(nI151):.2f},\n relative std = {1e2*np.std(nI151)/np.mean(nI151):.2f} %', color=cm.coolwarm(0.))
        plt.stairs(H[0], H[1], color=cm.coolwarm(0.), linewidth=8)
        plt.xlabel(r'$\mathcal{N}_{\mathrm{IPC15.1}}/\hat{\mathcal{N}}_{\mathrm{IPC15.1}}$')
        plt.legend()
        plt.grid()
