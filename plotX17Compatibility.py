...
fileDir = "./results/"
yields = np.array([74])
masses = np.array([16.97])
fractions = np.array([1])
_files = os.listdir(fileDir)
files = []
files = [f for f in _files if f.startswith(f"testX17_2023") and f.endswith(".txt")]
...
DATALR = _lr
ax.hist(lr, bins=None, color='b', alpha=0.5, label='Toys above data: %.2f %%' %(len(lr[lr >= DATALR])/len(lr)*100))
ax.vlines(DATALR, 0, 100000, color='r', label='Data')

plt.ylim(0.5, 2e4)
print('Number toys: %d' %(len(lr)))
print('Number toys >= DATALR (%.2f): %d' %(DATALR, len(lr[lr >= DATALR])))
